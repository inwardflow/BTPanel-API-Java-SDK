 ...
 * @since 2.0.0
 */
public class StopWebsiteApi extends BaseBtApi<BtResult<Boolean>> {
    ...
    public BtResult<Boolean> parseResponse(String response) {
        ...
        try {
            ...
            result.setMsg(json.getStr("msg", status ? "停止成功" : "停止失败"));
            result.setData(status);
            
            return result;
        } catch (Exception e) {
            throw new BtApiException("Failed to parse stop website response: " + e.getMessage(), e);
        }
    }
}