 ...
 * @since 2.0.0
 */
public class CompressFileApi extends BaseBtApi<BtResult<Boolean>> {
     ...
     * 设置压缩格式
     * 
     * @param format 压缩格式，如 zip, tar, tar.gz, tar.bz2
     * @return 当前API实例，支持链式调用
     */
    ...
    protected boolean validateParams() {
        return params.containsKey("path") && 
               ...
               !((String) params.get("filename")).isEmpty() &&
               params.get("format") != null && 
               !((String) params.get("format")).isEmpty();
    }
    ...
    public BtResult<Boolean> parseResponse(String response) {
        ...
        try {
            ...
            result.setMsg(json.getStr("msg", status ? "压缩成功" : "压缩失败"));
            result.setData(status);
            
            return result;
        } catch (Exception e) {
            throw new BtApiException("Failed to parse compress file response: " + e.getMessage(), e);
        }
    }
}