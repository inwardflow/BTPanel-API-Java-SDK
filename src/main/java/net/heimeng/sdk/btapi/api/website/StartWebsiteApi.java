 ...
 * @since 2.0.0
 */
public class StartWebsiteApi extends BaseBtApi<BtResult<Boolean>> {
    ...
    public BtResult<Boolean> parseResponse(String response) {
        ...
        try {
            ...
            result.setMsg(json.getStr("msg", status ? "启动成功" : "启动失败"));
            result.setData(status);
            
            return result;
        } catch (Exception e) {
            throw new BtApiException("Failed to parse start website response: " + e.getMessage(), e);
        }
    }
}