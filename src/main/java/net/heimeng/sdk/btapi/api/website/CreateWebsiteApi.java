 ...
 * 创建网站API实现
 *
 * <p>用于在宝塔面板中创建新的网站，支持配置域名、路径、FTP、数据库等。
 *
 * @author InwardFlow
 * @since 2.0.0
 */
public class CreateWebsiteApi extends BaseBtApi<BtResult<CreateWebsiteResult>> {
   ...
   * @return 当前API实例，支持链式调用
   */
  public CreateWebsiteApi setDatabaseCredentials(
      String databaseUser, String databasePassword, String charset) {
    if (StrUtil.isEmpty(databaseUser)
        || StrUtil.isEmpty(databasePassword)
        || StrUtil.isEmpty(charset)) {
