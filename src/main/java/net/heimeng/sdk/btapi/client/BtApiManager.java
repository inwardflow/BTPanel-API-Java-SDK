...
public class BtApiManager {
     ...
     */
    public <T> CompletableFuture<T> executeAsyncFuture(BtApi<T> api) {
    ...
    public <T> T executeAsyncWithTimeout(BtApi<T> api, long timeout, TimeUnit unit) {
        try {
            return executeAsyncFuture(api).get(timeout, unit);
    ...
    /**
     * 执行异步API请求，返回 CompletableFuture，支持超时控制
     *
     * @param <T> API返回数据类型
     * @param api API接口实例
     * @param timeout 超时时间
    ...
    public <T> CompletableFuture<T> executeAsyncWithTimeoutFuture(BtApi<T> api, Duration timeout) {
        ...
        executeAsyncFuture(api).whenComplete((result, throwable) -> {
            if (throwable != null) {
